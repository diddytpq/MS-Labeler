...
import cv2
import torch
from utils import check_LLM, get_yolo_label, get_zero_shot_label, build_transform, plot_one_box, nms_test, make_square_bbox, dynamic_preprocess, merge_overlapping_boxes
...
def get_bboxes(mask):
    """
    이진 마스크에서 경계 상자를 얻는 함수.

    Args:
    mask (numpy.ndarray): 이진 마스크 (numpy 배열)

    Returns:
    list of tuple: 경계 상자의 리스트 (x, y, w, h)
    """
...
def overlay_mask_on_image(image, mask, color):
    """
    이진 마스크를 원본 이미지 위에 덮어쓰기.

    Args:
    image (numpy.ndarray): 원본 이미지
    mask (numpy.ndarray): 이진 마스크
    color (numpy.ndarray): 색상 배열 (RGBA)

    Returns:
    numpy.ndarray: 마스크가 덮어쓰여진 이미지
    """
    overlay = image.copy()
    alpha = color[3]  # 투명도
    for i in range(3):
...
with torch.no_grad():
    ...
    for frame_num, label in yolo_label_data.items():
        all_boxes = yolo_label_data[frame_num] + zeroshot_label_data[frame_num]

        nms_boxes, non_nms_boxes = nms_test(yolo_label_data[frame_num], zeroshot_label_data[frame_num], iou_threshold=0.75)
        ...
        non_llm_input_bboxes[frame_num] = nms_boxes_original_format
        llm_input_bboxes[frame_num] = non_nms_boxes_original_format
    ...
    for frame_num, img in img_buffer.items():
        before_bbox = non_llm_input_bboxes[frame_num] + LLM_label[frame_num]
        bboxes_list[frame_num] = merge_overlapping_boxes(before_bbox, iou_threshold = 0.5)
