...
def split_video_to_image(video_path, train_data_video_name):
    ...
    # 초당 2장 추출을 위한 프레임 간격 계산
    frame_interval = fps // 1  # 1초당 2장
    ...
    while True:
        ...
        if frame_count % frame_interval == 0:
            ...
            cv2.imwrite(os.path.join(current_save_dir, img_name), frame)
            saved_count += 1
...
def get_yolo_label(model_path:str,buffer:list, init_model = False) -> dict:
    ...
    for i in range(0, len(images), batch_size):
        ...
        # 배치로 예측 수행
        if init_model:
            results = model.predict(batch_images, classes = [0, 1, 2, 3, 5, 7], imgsz=640, conf=0.25, iou=0.5, verbose=False)
            # 각 결과 처리
            ...
            results = model.predict(batch_images, imgsz=640, conf=0.25, iou=0.5, verbose=False)
            for j, result in enumerate(results):
                frame_num = batch_frames[j]
                boxes = result.boxes.data.cpu().numpy().astype(float)
...
def parse_assistant_response(response_text):
    ...
    # 방법 1: 마지막 assistant를 찾아서 다음 텍스트 추출
    if "assistant" in response_text.lower():
        ...
        if assistant_index != -1:
            ...
            # print(f"After last assistant: {repr(after_assistant)}")
            ...
            if words:
                result = words[0].strip('.,!?')  # 문장부호 제거
                # print(f"Word extraction result: {repr(result)}")
                return result
    
    # 방법 2: 정규표현식으로 마지막 assistant 다음의 첫 번째 단어 추출
    assistant_pattern = r'.*assistant\s*([A-Za-z]+)'
    assistant_match = re.search(assistant_pattern, response_text, re.IGNORECASE | re.DOTALL)
    
    if assistant_match:
        result = assistant_match.group(1).strip()
        # print(f"Regex word result: {repr(result)}")
        return result
    
    # 방법 3: 줄 단위로 분할하여 마지막 assistant가 포함된 줄에서 첫 번째 단어 추출
    ...
    for i in range(len(lines)-1, -1, -1):  # 뒤에서부터 검색
        ...
        if 'assistant' in line.lower():
            ...
            if words:
                ...
                # print(f"Line word result: {repr(result)}")
                return result
    
    # 방법 4: 문자열 분할 후 마지막 부분에서 첫 번째 단어 추출
    parts = response_text.split("assistant")
    if len(parts) > 1:
        words = parts[-1].strip().split()
        if words:
            result = words[0].strip('.,!?')
            # print(f"Split word result: {repr(result)}")
            return result
    ...
    for frame_num, img in enumerate(img_buffer):
        ...
        for cls, x1, y1, x2, y2, score in bboxes:
            ...
            generated_ids = model.generate(
                    **inputs, 
                    max_new_tokens=5,  # 토큰 수 제한으로 메모리 사용량 조절
